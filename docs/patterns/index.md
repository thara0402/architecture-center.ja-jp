...
<tr>
    ...
    <td>
        ...
        <p>データ管理はクラウド アプリケーションの重要な要素であり、品質属性のほとんどに影響します。 通常、パフォーマンス、スケーラビリティ、または可用性の理由から、データは複数のサーバーにまたがってさまざまな場所でホスティングされます。これによって、広範な課題が生じることがあります。 たとえば、データの整合性を維持する必要があります。また、通常はさまざまな場所にあるデータを同期する必要があります。</p>
    </td>
</tr>
<tr>
    ...
    <td>
        ...
        <p>優れた設計に含まれる要素には、コンポーネントの設計およびデプロイの一貫性や統一性、管理および開発を容易にする保全性、コンポーネントやサブシステムを他のアプリケーションやシナリオで利用できる再使用可能性などがあります。 設計および実装フェーズで下される決定は、クラウドでホストされるアプリケーションおよびサービスの品質と総保有コストに大きな影響を与えます。</p>
    </td>
</tr>
<tr>
    ...
    <td>
        ...
        <p>クラウド アプリケーションは、分散しているという特性から、コンポーネントとサービスをつなぐメッセージング インフラストラクチャを必要とします。これは、スケーラビリティを最大限にできるように、弱い結び付きであることが理想です。 非同期メッセージングは広く使用されており、多数の利点がありますが、メッセージの順序付け、有害メッセージ管理、べき等性などの課題もあります。</p>
    </td>
</tr>
<tr>
...
<tr>
    ...
    <td>
        ...
        <p>回復性とは、障害を正常に処理して復旧するシステムの機能です。 アプリケーションが多くの場合はマルチテナントであり、共有プラットフォーム サービスを使用し、リソースと帯域幅を競合し、インターネットで通信を行い、コモディティ ハードウェア上で実行するという、クラウド ホスティングの特性は、一時的な障害と永続的な障害の両方が発生しやすくなることを意味します。 迅速かつ効率的に障害を検出して復旧するにことが、回復性を維持するために必要です。</p>
    </td>
</tr>
<tr>
...
| [破損対策レイヤー](./anti-corruption-layer.md) | 最新アプリケーションと従来システムの間にファサードすなわちアダプター レイヤーを実装します。 |
...
| [外部構成ストア](./external-configuration-store.md) | アプリケーション展開パッケージの構成情報を一元管理される場所に移動します。 |
...
| [再試行](./retry.md) | 予測される一時的な障害をアプリケーションが処理できるようにします。アプリケーションがサービスまたはネットワーク リソースに接続しようとする際に、失敗した操作を透過的に再試行します。 |
...
| [静的コンテンツ ホスティング](./static-content-hosting.md) | 静的なコンテンツを、クライアントに直接配信できクラウドベースの記憶域サービスにデプロイします。 |
| [ストラングラー](./strangler.md) | 機能の各部を徐々に新しいアプリケーションやサービスで置き換えることで、従来のシステムを段階的に移行します。 |
| [調整](./throttling.md) | アプリケーションのインスタンス、個々のテナント、またはサービス全体によって使用されるリソースの使用量を制御します。 |
| [バレット キー](./valet-key.md) | 特定のリソースまたはサービスへの限定的な直接アクセスをクライアントに提供するトークンまたはキーを使用します。 |
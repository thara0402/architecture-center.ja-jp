...
このアーキテクチャは、「[Run Windows VMs for an N-tier application](n-tier.md)」(N 層アプリケーションでの Windows VM の実行) で説明したアーキテクチャ上に構築されています。 
...
詳細については、[Azure の N 層アーキテクチャでの Windows VM の実行](n-tier.md)に関する記事を参照してください。

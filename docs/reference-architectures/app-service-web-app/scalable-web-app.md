...
* **Web ジョブ**。 実行時間が長いタスクをバックグラウンドで実行するには、[Azure Web ジョブ][webjobs] を使用します。 Web ジョブは、スケジュールを指定して、継続して、またはメッセージがキューに格納されるなどのトリガーに応答して実行できます。 Web ジョブは、App Service アプリのコンテキストでバックグラウンド プロセスとして実行されます。

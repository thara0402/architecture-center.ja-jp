...
* Virtual Machine Scale Sets 内の Virtual Machines には、ブロックチェーン用の検証コントロール プロセスをホストするオンデマンドの計算機能があります
...
このシナリオの例を確認するには、Azure 上に [Ethereum PoA ブロックチェーン デモ アプリケーション][deploy]をデプロイし、[シナリオ ソース コードの README][source] の手順を実行します。

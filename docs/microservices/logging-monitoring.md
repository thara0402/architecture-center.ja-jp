...
何が起きているのかを把握するには、アプリケーションでテレメトリ イベントを生成する必要があります。 このイベントは、メトリックとテキストベースのログに分類できます。 

*メトリック*は、解析できる数値です。 メトリックを使用すると、リアルタイムで (またはほぼリアルタイムで) システムを観察したり、時間の経過と共にパフォーマンスの傾向を解析したりすることができます。 次のようなメトリックがあります。

- ノードレベルのシステム メトリック。CPU、メモリ、ネットワーク、ディスク、ファイル システムの使用状況などです。 システム メトリックは、クラスター内の各ノードのリソース割り当てを把握し、異常値を解決するために役立ちます。
 
- Kubernetes のメトリック。 サービスはコンテナーで実行されるため、VM レベルだけでなく、コンテナー レベルでメトリックを収集する必要があります。 Kubernetes では、cAdvisor (Container Advisor) がエージェントであり、各コンテナーで使用される CPU、メモリ、ファイル システム、およびネットワーク リソースに関する統計を収集します。 kubelet デーモンは、cAdvisor からリソース統計を収集し、REST API を介して公開します。
   
- アプリケーション メトリック。 サービスの動作の把握に関連するメトリックが含まれます。 たとえば、受信した HTTP 要求の数、要求の待機時間、メッセージ キューの長さ、1 秒あたりに処理されるトランザクション数などです。

- 依存サービスメトリック。 クラスター内のサービスから、管理対象の PaaS サービスなど、クラスター外にある外部サービスが呼び出されることがあります。 Azure サービスは、[Azure Monitor](/azure/monitoring-and-diagnostics/monitoring-overview) を使用して監視できます。 サードパーティのサービスの場合、メトリックが提供される場合と提供されない場合があります。 提供されない場合、待機時間とエラー率の統計を追跡するには、独自のアプリケーション メトリックに頼る必要があります。

*ログ*は、アプリケーションの実行中に発生するイベントの記録です。 アプリケーション ログ (トレース ステートメント) や Web サーバー ログなどが含まれます。 ログは、主に科学捜査と根本原因の解析に役立ちます。 

...
回復性のあるアプリケーションを設計するには、発生する可能性のあるさまざまな障害モードに対して計画し、障害を緩和する必要があります。 アプリケーションの設計をこのチェックリストの項目で確認して、回復性を向上させてください。
...
### <a name="sql-server-running-in-a-vm"></a>SQL Server (VM で実行)

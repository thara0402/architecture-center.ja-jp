...
- クラウド サービス:[クラウド サービスの自動スケールの方法][cloud-service-autoscale]
...
**必要に応じてリモート操作用の回復性のパターンを実装します。** アプリケーションがリモート サービス間の通信に依存する場合は、[再試行パターン](../patterns/retry.md)や[サーキット ブレーカー パターン](../patterns/circuit-breaker.md)などの一時的な障害に対処する[設計パターン](../patterns/category/resiliency.md)に従ってください。

...
さらに、組織による Azure リソースの使用を管理できるように、Azure ガバナンス ツールを構成する方法についても学習し始めました。 シンプルなワークロードのデプロイに必要なリソースへの、1 つのチームによるアクセスを管理する方法を説明しました。 実際の組織では、複数のチームが、複数のワークロードを同時に処理しています。 

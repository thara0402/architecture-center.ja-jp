...
これは、"price" を更新し、"color" を削除し、"size" を追加するようサーバーに指示しています。 "name" と "category" は変更されません。 JSON マージ パッチの詳細については、[RFC 7396](https://tools.ietf.org/html/rfc7396) を参照してください。 JSON マージパッチのメディアの種類は "application/merge-patch+json" です。
